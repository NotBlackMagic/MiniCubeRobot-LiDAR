...
//	while(LL_RCC_HSI_IsReady() != 1);
	...
	LL_RCC_SetAPB1Prescaler(LL_RCC_APB1_DIV_2);			//APB1 Peripheral Clock (PCLK1): HCLK / DIV = 72MHz/2 = 36MHz
	LL_RCC_SetAPB2Prescaler(LL_RCC_APB2_DIV_1);			//APB2 Peripheral Clock (PCLK2): HCLK / DIV = 72MHz/1 = 72MHz
	LL_RCC_SetSysClkSource(LL_RCC_SYS_CLKSOURCE_PLL);
	while(LL_RCC_GetSysClkSource() != LL_RCC_SYS_CLKSOURCE_STATUS_PLL);		//Wait till System clock is ready

	LL_SetSystemCoreClock(72000000);
	LL_RCC_SetADCClockSource(LL_RCC_ADC_CLKSRC_PCLK2_DIV_6);				//Set ADC Clock (have to be <= 14MHz)
